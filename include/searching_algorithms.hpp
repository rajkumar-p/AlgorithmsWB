                                   ...
                                   int begin, int end, T key)
{
    ...
    int middle = begin + ((end - begin) / 2);
    if (elements[middle] <= key) {
        return binary_search_rightmost_helper(elements, middle + 1, end, key);
    } else {
        return binary_search_rightmost_helper(elements, begin, middle - 1, key);
    }
}
